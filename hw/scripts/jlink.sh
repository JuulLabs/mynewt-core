...
jlink_load () {
    ...
    #
    echo "shell sh -c \"trap '' 2; $JLINK_GDB_SERVER $EXTRA_JTAG_CMD -device $JLINK_DEV -speed 1000 -if SWD -port 3333 -singlerun &\" " > $GDB_CMD_FILE
...
jlink_debug() {
    ...
    if [ -z "$NO_GDB" ]; then
        ...
        if [ $WINDOWS -eq 1 ]; then
            ...
            #
            $COMSPEC /C "start $COMSPEC /C $JLINK_GDB_SERVER $EXTRA_JTAG_CMD -device $JLINK_DEV -speed 1000 -if SWD -port 3333 -singlerun"
        else
            #
            # Block Ctrl-C from getting passed to jlink server.
            #
            set -m
            $JLINK_GDB_SERVER $EXTRA_JTAG_CMD -device $JLINK_DEV -speed 1000 -if SWD -port 3333 -singlerun > /dev/null &
            set +m
        fi
	...
	if [ $WINDOWS -eq 1 ]; then
        ...
        $JLINK_GDB_SERVER $EXTRA_JTAG_CMD -device $JLINK_DEV -speed 1000 -if SWD -port 3333 -singlerun
    fi
    return 0
}
